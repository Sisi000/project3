...
test("Used for testing euclidean distance calculation", () => {
  ...
  let testResult1 = euclideandistance(
    coord1[0],
    coord1[1],
    coord1[2],
    coord2[0],
  ...
  let testResult2 = euclideandistance(
    coord3[0],
    coord3[1],
    coord3[2],
    coord4[0],
  ...
  let testResult3 = euclideandistance(
    coord5[0],
    coord5[1],
    coord5[2],
    coord6[0],
...
test("Used for testing filter", () => {
  ...
  for (let data of glassesTestData1) {
    //No filters
...
/*
test("Used for building/testing datasets", async () => {
  let imageData = [
    "vanessa-hudgens.jpg",
    "angelina-jolie.jpg",
    "david.jpg",
  ...
  let directory = "./testimages/data-2022-06-24/"
  let results = {};
  async function getGoogleInfo(imageData,directory) {
    await imageData.forEach(async (file) => {
      let imageFileUpload = fs.readFileSync(
        directory+file
      );
      let resultGoogle = await uploadImageTest(imageFileUpload);
      let userData = await calculateUserData(resultGoogle);
      results.file = userData
      console.log(results)
    });
  }
  await getGoogleInfo(imageData, directory)
  ...
  write(results, "./testimages/data-2022-06-24/data.txt");
});
*/
